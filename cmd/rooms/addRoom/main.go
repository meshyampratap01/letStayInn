...
func handler(ctx context.Context, req events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	if !ok || roleStr != "Manager" {
		body, _ := json.Marshal(response.NewErrorResponse(constants.StatusForbidden, "Only managers can add rooms"))
	...
	err := roomSvc.AddRoom(reqBody.Number, reqBody.Type, reqBody.Price, true, reqBody.Description)
	if err != nil {
	...
	resp := response.NewSuccessResponse(constants.StatusCreated, "Room created successfully", nil)
