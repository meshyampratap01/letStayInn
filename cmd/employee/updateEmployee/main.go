...
func handler(ctx context.Context, req events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	employeeEmail := req.PathParameters["employeeEmail"]
	if employeeEmail == "" {
		body, _ := json.Marshal(response.NewErrorResponse(constants.StatusBadRequest, "Missing employee email"))
		return events.APIGatewayProxyResponse{
	...
	if err := json.Unmarshal([]byte(req.Body), &availReq); err != nil {
	...
	err := managerSvc.UpdateEmployeeAvailability(employeeEmail, availReq.Available)
	if err != nil {
	...
	resp := response.NewSuccessResponse(constants.StatusOK, "Employee availability updated", nil)
